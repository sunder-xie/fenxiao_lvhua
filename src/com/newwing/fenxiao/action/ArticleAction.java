...
public class ArticleAction extends BaseAction {
	...
	public void list() {
		...
		if (("".equals(key)) || (key == null)) {
			...
			count = this.articleService.getTotalCount("from Article where title like '%" + key + "%' and deleted=0",
					new Object[0]);
		}
		...
		List list = this.articleService.list("from Article where deleted=0 order by id desc", this.page.getStart(),
		...
		try {
			...
			if ((idStr == null) || ("".equals(idStr))) {
				...
				if (findArticle == null) {
					...
					List<ArticleCate> articleCatelist = this.articleCateService
							.list("from ArticleCate where deleted=0");
					String zNodes = "";
					for (ArticleCate articleCate : articleCatelist) {
	...
	public void update() {
		...
		try {
			...
			if (this.article == null) {
				...
				Article findArticle = (Article) this.articleService.findById(Article.class,
						this.article.getId().intValue());
