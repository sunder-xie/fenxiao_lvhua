...
public class ArticleCateAction extends BaseAction {
	...
	public void list() {
		...
		for (ArticleCate articleCate : list) {
			zNodes = zNodes + "<li data-id='" + articleCate.getId() + "' data-pid='" + articleCate.getFatherId()
					+ "' data-tabid='" + articleCate.getId() + "'>" + articleCate.getName() + "[ID:"
					+ articleCate.getId() + "]</li>";
		}
	...
	public void add() {
		...
		for (ArticleCate articleCate : list) {
			zNodes = zNodes + "<li data-id='" + articleCate.getId() + "' data-pid='" + articleCate.getFatherId()
					+ "' data-tabid='" + articleCate.getId() + "'>" + articleCate.getName() + "</li>";
		}
		this.cfg = new Configuration();
		...
		try {
			if ((idStr == null) || ("".equals(idStr))) {
				...
				if (findArticleCate == null) {
					...
					String zNodes = "<li data-id='0' data-pid='0' data-tabid='0'>顶级栏目</li>";
					for (ArticleCate articleCate : list) {
						zNodes = zNodes + "<li data-id='" + articleCate.getId() + "' data-pid='"
								+ articleCate.getFatherId() + "' data-tabid='" + articleCate.getId() + "'>"
								+ articleCate.getName() + "</li>";
					}

					String fatherName = "";
					if (findArticleCate.getFatherId() != 0) {
						ArticleCate fatherArticleCate = (ArticleCate) this.articleCateService
								.findById(ArticleCate.class, findArticleCate.getFatherId());
						if (fatherArticleCate != null)
							fatherName = ((ArticleCate) this.articleCateService.findById(ArticleCate.class,
									findArticleCate.getFatherId())).getName();
						else
							fatherName = "上级栏目不存在";
					} else {
						fatherName = "顶级栏目";
					...
					FreemarkerUtils.freemarker(this.request, this.response, "articleCateEdit.ftl", this.cfg,
							(Map) root);
				}
			}
		} catch (JSONException e) {
	...
	public void update() {
		...
		try {
			if (this.articleCate == null) {
				...
				ArticleCate findArticleCate = (ArticleCate) this.articleCateService.findById(ArticleCate.class,
						this.articleCate.getId().intValue());
