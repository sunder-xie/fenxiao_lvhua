...
public class FinancialAction extends BaseAction {
	...
	public void list() {
		...
		List financialList = this.financialService.list(hql, this.page.getStart(), this.page.getPageSize(),
				new Object[0]);
			...
			if (this.financial == null) {
				...
				if (findUser == null) {
					...
					findUser.setBalance(Double
							.valueOf(findUser.getBalance().doubleValue() + this.financial.getMoney().doubleValue()));
	...
	public void update() {
		...
		try {
			if (this.financial == null) {
				...
				Financial findfinancial = (Financial) this.financialService.findById(Financial.class,
						this.financial.getId().intValue());
